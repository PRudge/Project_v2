...
class Country
  ...
  def self.cities_visited(id)
    sql = "SELECT * FROM cities
    WHERE country_id = $1 AND visit_status = 0 ORDER BY name"
    values = [id]
    city_data = SqlRunner.run(sql, values)
    cities = city_data.map {|city| City.new(city) }
    return cities
  ...
  def self.cities_not_visited(id)
    sql = "SELECT * FROM cities
    WHERE country_id = $1 AND visit_status = 1 ORDER BY name"
    values = [id]
    city_data = SqlRunner.run(sql, values)
    cities = city_data.map {|city| City.new(city) }
    return cities
